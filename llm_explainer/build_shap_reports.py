...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=25,
        help="How many SHAP-grounded cases to select.",
    )
    parser.add_argument(
        "--top-k-features",
        type=int,
...
def feature_effect_text(shap_value: float) -> str:
    if shap_value > 0:
        return "pushes the prediction toward higher AUC (relative resistance)"
    if shap_value < 0:
        return "pushes the prediction toward lower AUC (relative sensitivity)"
...
def enrich_feature_row(
    ...
    *,
    ...
    effect_text = feature_effect_text(shap_value)
...
def build_framework_components(payload: dict[str, Any]) -> dict[str, str]:
    drug = payload["drug"]["name"]
    cell = payload["cell_line"]["name"]
    return {
        "component_1_sample_context": (
            f"Explain the observed response of {drug} on {cell} using observed AUC, the RF-predicted AUC, "
            "and the local drug/cell cohort baselines."
        ),
        "component_2_shap_evidence": (
            "Prioritize the top TreeSHAP features, explicitly stating whether each feature pushes the prediction "
            "toward higher AUC/resistance or lower AUC/sensitivity."
        ),
...
def render_markdown_report(payload: dict[str, Any]) -> str:
    ...
    lines.append(f"- Observed AUC: **{payload['query_y_true']:.4f}**")
    lines.append(f"- RF-predicted AUC: **{payload['model_predicted_auc']:.4f}**")
    lines.append(f"- Prediction error (observed - predicted): **{payload['prediction_error']:+.4f}**")
    lines.append(f"- Global mean AUC across all pairs: **{payload['global_mean_auc']:.4f}**")
    lines.append(
...
def build_payload_for_case(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "selection_reason": selection_reason_text(prediction_error),
        "drug": {
    ...
    payload["framework_components"] = build_framework_components(payload)
    return payload
...
def main() -> None:
    ...
    print(f"Selecting {args.num_reports} diverse SHAP-grounded cases...")
    selected_sample_ids = select_diverse_cases(
        sample_summary,
    ...
    if len(selected_sample_ids) < args.num_reports:
        raise RuntimeError(
            f"Only selected {len(selected_sample_ids)} cases; expected {args.num_reports}."
        )

    md_parts = ["# SHAP-Grounded Sample Reports", ""]
